...
export class AccessStore {
  ...
  reserve(id, kind, input) {
    ...
    const limits = [
      [`${id}:jobs`, date.slice(0, 13), 1, 20],
      [`${id}:ai`, date.slice(0, 10), ai, 20], ['global:ai', date.slice(0, 10), ai, 60],
      [`${id}:search`, date.slice(0, 10), search, 60], ['global:search', date.slice(0, 10), search, 120],
    ];
